...
class ProfilesController extends Controller
{
    ...
    public function upload(Request $request)
    {
        if (Input::hasFile('file')) {
            $path = $request->file->storeAs('avatars',
                'avatar_' . auth()->user()->id . '.' . $request->file->extension());
            if (!auth()->user()->profile) {
                auth()->user()->profile()->save(new Profile(['avatar' => $path]));
            } else {
    ...
    public function userProfileAvatar($id, $image)
    {
        ...
        return Image::make(storage_path($user->profile->avatar))->response();
    }
    ...
    {
        return view('profiles.role-permission',[
            'user' => auth()->user()
        ]);
    }
    ...
    public function updatePassword(Request $request)
    {
        ...
        return redirect('/profile/change-password')->with('success', 'le mot de passe a été changé avec succès.');
    }
}
