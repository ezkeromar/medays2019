            ...
            <table class="kt-datatable" id="html_table4" width="100%">
                <thead>
                    <tr>
                        ...
                        <th title="Field #3">Aéroport Arrivée</th>
                        <th title="Field #4">Aéroport Provenance</th>
                        <th title="Field #5">Date Arrivée</th>
                        <th title="Field #6">Heure Arrivée</th>
                ...
                @foreach ($participants as $participant)
                    <tr>
                        ...
                        <td>
                        @foreach(config('meDays.airports') as $item)
                        @if($item['id']==$participant->arrival_recovery_point)
                            {{$item['name']}}
                        @endif
                        @endforeach
                        </td> 
                        <td>{{$participant->arrival_airport}}</td>
                        <td>
                        @php
                        try {
                            echo $participant->transfer_arrival_date ? \Carbon\Carbon::parse($participant->transfer_arrival_date)->format("d/m/Y") : "";
                        } catch(Exception $e) {
                            echo $participant->transfer_arrival_date;
                        }
                        @endphp
                        </td>
                        <td>{{!empty($participant->transfer_arrival_time)?Carbon\Carbon::createFromFormat("H:i:s", $participant->transfer_arrival_time)->format("H:i"):'' }}</td>
